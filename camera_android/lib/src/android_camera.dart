...
class AndroidCamera extends CameraPlatform {
  ...
  Future<List<CameraResolution>> getAvailableResolutions(
    ...
    try {
      // Get camera description to extract camera name
      ...
      final String cameraName = cameras[cameraId].name;
      final List<PlatformCameraResolution> platformResolutions =
          await _hostApi.getAvailableResolutions(cameraName);

      return platformResolutions.map((res) => CameraResolution(
        width: res.width,
        height: res.height,
        minFps: res.minFps,
        maxFps: res.maxFps,
  ...
  @override
  Future<List<CameraResolution>> getAvailableVideoResolutions(int cameraId) async {
    try {
      final String cameraName = cameraId.toString();
      final List<PlatformCameraResolution> resolutions =
          await _hostApi.getAvailableVideoResolutions(cameraName);

      return resolutions
          .map((PlatformCameraResolution r) => CameraResolution(
                width: r.width,
                height: r.height,
                minFps: r.minFps,
                maxFps: r.maxFps,
