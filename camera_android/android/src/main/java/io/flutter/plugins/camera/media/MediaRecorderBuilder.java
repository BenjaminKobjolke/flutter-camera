...
public class MediaRecorderBuilder {
  ...
  private final android.util.Size customVideoSize;
  ...
  /**
   * Constructor for manual configuration with custom video size (fallback).
   * Used when no CamcorderProfile or EncoderProfiles is available.
   *
   * @param customVideoSize Custom video resolution (width x height)
   * @param parameters Recording parameters
   */
  public MediaRecorderBuilder(
      @NonNull android.util.Size customVideoSize,
      @NonNull RecordingParameters parameters) {
    this.customVideoSize = customVideoSize;
    this.camcorderProfile = null;
    this.encoderProfiles = null;
    this.recorderFactory = new MediaRecorderFactory();
    this.parameters = parameters;
  }
  ...
  public MediaRecorder build() throws IOException, NullPointerException, IndexOutOfBoundsException {
    ...
    if (SdkCapabilityChecker.supportsEncoderProfiles() && encoderProfiles != null) {
    ...
    } else if (customVideoSize != null) {
      // Manual configuration fallback when no profile is available
      ...
      if (enableAudio) {
        ...
        mediaRecorder.setAudioEncodingBitRate(
            (parameters.audioBitrate != null && parameters.audioBitrate.intValue() > 0)
                ? parameters.audioBitrate
                : 128000); // Default 128kbps for audio
      ...
      mediaRecorder.setVideoEncodingBitRate(
          (parameters.videoBitrate != null && parameters.videoBitrate.intValue() > 0)
              ? parameters.videoBitrate
              : 10000000); // Default 10Mbps for video
      mediaRecorder.setVideoFrameRate(
          (parameters.fps != null && parameters.fps.intValue() > 0)
              ? parameters.fps
              : 30); // Default 30fps
      mediaRecorder.setVideoSize(customVideoSize.getWidth(), customVideoSize.getHeight());
    } else {
      // No profile and no custom size - this should not happen
      throw new IllegalStateException(
          "MediaRecorderBuilder requires either a CamcorderProfile, EncoderProfiles, or custom video size");
    }
